...
public class MarchingCubes_Test : MonoBehaviour
{
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int z = 0; z < width; z++)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        Vector3Int corner = new Vector3Int(x, y, z) + MarchingTable_Test.Corners[i];
    ...
    private void MarchCube (Vector3 position, float[] cubeCorners)
    {
        ...
        for (int t = 0; t < 5; t++)
        {
            for (int v = 0; v < 3; v++)
            {
                int triTableValue = MarchingTable_Test.Triangles[configIndex, edgeIndex];
                ...
                Vector3 edgeStart = position + MarchingTable_Test.Edges[triTableValue, 0];
                Vector3 edgeEnd = position + MarchingTable_Test.Edges[triTableValue, 1];
